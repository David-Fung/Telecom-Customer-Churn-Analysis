...
SELECT 
	...
	ROUND(COUNT(*)/(SELECT COUNT(*) FROM customer_data)::DECIMAL, 2) AS percent_customers
FROM customer_data
...
SELECT 
	...
	churn_reason, 
	COUNT(*) AS num_customers, 
	ROUND(COUNT(*)/(SELECT COUNT(*) FROM customer_data WHERE churn_reason IS NOT null)::DECIMAL, 2) AS percent_customers
FROM customer_data 
